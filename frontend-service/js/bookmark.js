...
// The bookmark model
function Bookmark(selfHref, url, created, note, title) {
    var self = this;
            ...
            success: function (allData) {
                ...
                if (parsed._embedded) {
                    ...
                    var mappedBookmarks = $.map(parsedBookmarks, function (bookmark) {
                        return new Bookmark(bookmark._links.self.href, bookmark.url, bookmark.created, bookmark.note, bookmark.title)
                    });
